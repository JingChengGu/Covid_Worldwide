...
var app = (function () {
    ...
    const file$1 = "src/WorldMap.svelte";
    ...
    function create_fragment$1(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			attr_dev(svg_1, "viewBox", "200 200 700 700");
    			add_location(svg_1, file$1, 160, 0, 5938);
    			attr_dev(div, "class", "tooltip");
    			add_location(div, file$1, 162, 0, 6020);
    		},
    ...
    function instance$1($$self, $$props, $$invalidate) {
    	...
    	let daysCount;
    	let tooltip;
    	const colorScale = linear().domain([0, 10000, 50000, 100000, 500000, 1000000]).range(["#316754", "#F9F208", "#F5BE16", "#F56016", "#F51616", "#A10707"]);
    	...
    	function setProjectionAndPath() {
    		...
    		projection = mercator().center([0, 0]).scale(100).translate([550, 550]); // Adjust this to change the initial centering of the map, if necessary
    		// Set a default scale; this will be scaled according to the viewBox
    		// Translate to the center of the viewBox dimensions

    		path = index$2().projection(projection);
    	...
    	onMount(async () => {
    		worldData = await json('globe.geo.json');
    		const covidCsvData = await csv('Covid_data/full_grouped.csv');
    		covidData = processCovidData(covidCsvData);
    		dates = Object.keys(covidData[Object.keys(covidData)[0]]);
    		daysCount = dates.length - 1;

    		// map projection
    		projection = mercator().scale(100).translate([svg.clientWidth / 2, svg.clientHeight / 2]);

    		path = index$2().projection(projection);
    		$$invalidate(1, tooltip = select('.tooltip'));
    	...
    	function drawMap() {
    		const paths = select(svg).selectAll('path').data(worldData.features).enter().append('path').attr('d', path).attr('fill', d => {
    			const countryData = covidData[d.properties.name];

    			const data = countryData && countryData[dates[0]]
    			? countryData[dates[0]]
    			: { cases: 0, deaths: 0, recovered: 0 };

    			return colorScale(data.cases);
    		}).attr('stroke', '#85F5CC').attr('stroke-width', 0.5); // Set the stroke color to black
    		...
    		paths.on('mouseover', (event, d) => {
    			...
    			tooltip.style('left', `${event.pageX + 15}px`).style('top', `${event.pageY + 15}px`).style('visibility', 'visible').html(`<strong>${d.properties.name}</strong><br/>Cases: ${data.cases}<br/>Deaths: ${data.deaths}<br/>Recovered: ${data.recovered}`);
    		}).on('mousemove', event => {
    	...
    	function updateMap(currentDate) {
    		const paths = select(svg).selectAll('path');

    		// Transition colors
    		paths.transition().duration(500).attr('fill', d => {
    			const countryData = covidData[d.properties.name];

    			const data = countryData && countryData[currentDate]
    	...
    	$$self.$inject_state = $$props => {
    		...
    		if ('worldData' in $$props) worldData = $$props.worldData;
    		if ('covidData' in $$props) covidData = $$props.covidData;
    	...
    	return [svg, tooltip, svg_1_binding, div_binding];
    }
    ...
    function create_fragment(ctx) {
    	...
    	let worldmap;
    	let current;
    	worldmap = new WorldMap({ $$inline: true });

    	const block = {
    		c: function create() {
    			main = element("main");
    			h1 = element("h1");
    			h1.textContent = "WorldWide Covid Data";
    			t1 = space();
    			p = element("p");
    			t2 = text$1("A Visualizaiton of how covid took over the world, here is the data we used ");
    			a = element("a");
    			a.textContent = "Covid Data";
    			t4 = text$1(".");
    			t5 = space();
    			input = element("input");
    			t6 = space();
    		...
    		m: function mount(target, anchor) {
    			...
    			append_dev(main, t8);
    			mount_component(worldmap, main, null);
    			current = true;
    		},
    		p: noop$4,
